...
## Note: This script investigates this reviewer comment:
# P16 explains that hybrid choice models have three components. Note that this is not necessarily so. Structural equations are an optional part of these models. The bare minimum you would need for something called hybrid choice model is just the main probit equation including a LV and a set of measurement equations linking the indicators to the LV. So I wonder what the models looks like with only those two components?
## Author: Dr Peter King (p.m.king@kent.ac.uk)
## Last change: 31/05/2022
## TODO: add plots
...
## Normalisation
# database$Q13CurrentThreatToSelf = as.numeric(scale(database$Q13CurrentThreatToSelf))
# database$Q14FutureThreatToSelf = as.numeric(scale(database$Q14FutureThreatToSelf))
# database$Q15ThreatToEnvironment = as.numeric(scale(database$Q15ThreatToEnvironment))


# database$Q13CurrentThreatToSelf = database$Q13CurrentThreatToSelf-mean(database$Q13CurrentThreatToSelf)
# database$Q14FutureThreatToSelf = database$Q14FutureThreatToSelf-mean(database$Q14FutureThreatToSelf)
# database$Q15ThreatToEnvironment = database$Q15ThreatToEnvironment-mean(database$Q15ThreatToEnvironment)


# Estimate model:
apollo_control = list(
  modelName  = "Q1ICLV_NoME_2022_05_31",
  modelDescr = "Q1ICLV_NoME_2022_05_31",
  indivID    = "ID",
  mixing     = TRUE,
  nCores     = 1,
  noValidation=TRUE)
...
Q1ICLV_NoME_2022_05_31 = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

apollo_modelOutput(Q1ICLV_NoME_2022_05_31,modelOutput_settings = list(printPVal=TRUE))
xtable::xtable(round(data.frame(apollo_modelOutput(Q1ICLV_NoME_2022_05_31,modelOutput_settings = list(printPVal=TRUE))),3),digits=3)
apollo_saveOutput(Q1ICLV_NoME_2022_05_31,saveOutput_settings = list(printPVal=TRUE))


## Elicit conditionals and unconditionals:
Q1ICLV_NoME_2022_05_31_UCWTP <- apollo_unconditionals(Q1ICLV_NoME_2022_05_31,apollo_probabilities,apollo_inputs)
Q1ICLV_NoME_2022_05_31_ConWTP <- apollo_conditionals(Q1ICLV_NoME_2022_05_31,apollo_probabilities,apollo_inputs)


## Save outputs
saveRDS(Q1ICLV_NoME_2022_05_31,"Q1ICLV_NoME_2022_05_31.rds")
saveRDS(Q1ICLV_NoME_2022_05_31_UCWTP,"Q1ICLV_NoME_2022_05_31_UCWTP.rds")
saveRDS(Q1ICLV_NoME_2022_05_31_ConWTP,"Q1ICLV_NoME_2022_05_31_ConWTP.rds")
# Q1ICLV_NoME_2022_05_31 <- readRDS("Q1ICLV_NoME_2022_05_31.rds")

# WTP:
# Model <- Q1ICLV_NoME_2022_05_31
...
# Q1ICLV_NoME_2022_05_31WTP <- median(-Model$estimate["intercept"]+CVunconditionals7F$LV/Model$estimate["b_bid"])
